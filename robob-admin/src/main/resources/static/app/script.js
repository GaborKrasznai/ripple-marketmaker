function formatKeyBid(key) {
	...
	if(key.startsWith('#')) {
    	...
    	var res = key.split("/");
    	return res[0] + '/' + res[1].substring(0, 4);
    }
}

function formatKeyAsk(key) {
	if(key === undefined) {
		return "";
	}
	if(key.startsWith('#')) {
    	...
    	var res = key.split("/");
    	return res[2] + '/' + res[1].substring(0, 4);
...
function formatKey(key) {
	...
	if(key.startsWith('#')) {
    	...
    	var res = key.split("/");
    	return res[0] + '/' + res[1].substring(0, 4) + ' - ' + res[2] + '/' + res[1].substring(0, 4);
    }
}

function isRipplExchange(key) {
    if(key === undefined) {
        return false;
    }
